...
function resource(item, included, responseModel) {
  ...
  _.forOwn(model.attributes, function (value, key) {
    if (isRelationship(value)) {
      deserializedModel[key] = attachRelationsFor.call(_this2, model, value, item, included);
    } else {
...
function attachHasOneFor(model, attribute, item, included) {
  ...
  var relatedItems = relatedItemsFor(model, attribute, item, included);
...
function relatedItemsFor(model, attribute, item, included) {
  var relationName = _.findKey(model.attributes, attribute);
  var relationMap = _.get(item.relationships, [relationName, 'data'], false);
  if (!relationMap) {
    return [];
  }
